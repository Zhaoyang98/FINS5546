...
COLUMNS = ['Volume', 'Date', 'Adj_Close', 'Close', 'Open', 'High']
...
def get_tics(pth):
    ...
    with open(pth, 'r') as f:
        for line in f:
            l = [item.strip('"').lower() for item in line.strip().split("=")]
            tic_dict[l[1]] = l[0]
    return tic_dict
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    with open(os.path.join(DATDIR, f"{tic}_prc.dat"), "r") as f:
...
def line_to_dict(line):
    ...
    result = {}
    left_bound = 0
    for column, width in COLWIDTHS.items():
        right_bound = left_bound + width
        result[column] = line[left_bound:right_bound]
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            raise Exception("Error: tickers_lst is an empty list.")
        for ticker in tickers_lst:
            if ticker.lower() not in tic_exchange_dic.keys():
                raise Exception(f"Error: '{ticker}' is not a key of tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        if len(col_lst) == 0:
            raise Exception("Error: col_lst is an empty list.")
        for col in col_lst:
            if col not in COLUMNS:
                raise Exception(f"Error: '{col}' is not found in `COLUMNS`.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for ticker in tickers_lst:
        data_list = []
        for l in read_dat(ticker):
            data_raw = line_to_dict(l)
            data_col = {col: data_raw[col] for col in col_lst}
            data_list.append(data_col)
        final[ticker] = {'exchange': tic_exchange_dic[ticker], 'data': data_list}
    return final
